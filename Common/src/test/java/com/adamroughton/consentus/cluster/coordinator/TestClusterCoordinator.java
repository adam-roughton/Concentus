...
import com.adamroughton.consentus.cluster.ClusterUtil;
...
public class TestClusterCoordinator extends TestClusterBase {
	...
	private ClusterCoordinator _clusterCoordinator;
	...
	public void setUp() {
		...
		_clusterCoordinator = new ClusterCoordinator(getZooKeeperAddress(), ROOT, COORDINATOR_ID, _exCallback);
	}
...
//	}
	...
	public void waitForReadyOneService() throws Throwable {
		...
		final ClusterCoordinator clusterCoordinator = _clusterCoordinator;
		ExecutorService executor = Executors.newCachedThreadPool();
		Future<?> waitingTaskFuture = executor.submit(new Runnable() {
			@Override
			public void run() {
				try {
					clusterCoordinator.waitForReady(participatingNodes);
	...
	public void waitForReadyManyServices() throws Throwable {
		...
		Future<?> waitingTaskFuture = executor.submit(new Runnable() {
			...
			public void run() {
				try {
					clusterCoordinator.waitForReady(partNodes);
				} catch (InterruptedException eInterrupted) {
					throw new RuntimeException(eInterrupted);
				}
			}
